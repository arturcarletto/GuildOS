...
The `guildmoderation` capability owns the first moderation action foundation: creating a Discord member communication timeout from the authenticated dashboard API, a dedicated privacy-safe moderation case history for successful timeout actions, and a live member search that helps operators select a target. It is intentionally narrow and does not include warnings, kick, ban, message deletion, queues, AI moderation, appeal workflows, realtime moderation streams, a persistent member directory or member profile table, avatar storage, role management, or moderation automation.

The dependency direction is:

`HTTP -> guildmoderation application service -> guildaccess authorization contract -> Discord moderation port -> moderation case recorder -> moderation_cases + guildaudit`
...
Only after Discord accepts the timeout does the service call `GuildModerationCaseRecorder`, which records one `MEMBER_TIMEOUT_CREATED` case and one matching `GuildAuditRecorder` event in the same short local transaction. Failed Discord actions do not create success cases or audit rows. The API response exposes only the Discord guild id, action type, target Discord user id, duration, and status. It does not expose internal UUIDs, operator ids, OAuth/session/token data, or the moderation reason. The case and audit summaries are application-generated and generic rather than raw reason text or user-facing Discord names.

`guild_os.moderation_cases` stores only a private database id, a public case id, the registered guild reference, action type (`MEMBER_TIMEOUT_CREATED` today), target type (`DISCORD_USER` today), target Discord user id, timeout duration, status (`COMPLETED` today), occurrence/creation timestamps, and a bounded Guild OS summary. It does not store raw reasons, member names, display names, usernames, avatars, OAuth/session/token data, raw Discord payloads, stack traces, or exception messages.

The case read path is:

`HTTP -> @AuthenticationPrincipal AuthenticatedOperator -> GuildAccessAuthorizer -> moderation case query -> JSON response`

`GET /api/v1/guilds/{discordGuildId}/moderation/cases` accepts `limit` (default 50, max 100), optional `actionType`, and optional inclusive `from` / exclusive `to` ISO instants. Missing guilds, missing access, and revoked access are indistinguishable `404` responses. Results are ordered newest first and expose only the Discord guild id plus public case id, action type, target type, target Discord user id, optional duration, status, summary, and occurrence time. The endpoint is read-only, requires authenticated active guild access, sends no CSRF token, and does not implement queues, appeals, AI moderation, kick/ban/warning/message-deletion flows, automation, or realtime streams.

`GET /api/v1/guilds/{discordGuildId}/moderation/members/search?query={query}&limit={limit}` adds a member search foundation over the same boundary. It is a live, authenticated lookup, not a persisted member directory: it exists only to help an authorized operator pick a moderation target. The controller takes operator identity from `@AuthenticationPrincipal AuthenticatedOperator`, and the same `GuildAccessAuthorizer.findActive` boundary produces the same non-enumerating `404` for missing guilds, missing access, and revoked access. The capability validates and normalizes the query into a `MemberSearchQuery` (required, trimmed, at least two characters unless it is a full snowflake, at most 64 characters; `limit` defaults to 10 and is capped at 25) and calls the `GuildModerationDiscordClient.searchMembers` port outside any database transaction. Only a full Discord snowflake (17-20 digits) becomes an exact id lookup returning at most one member; a purely numeric query that is not a full snowflake (such as `1` or `12`) is rejected with a controlled `400` rather than run as a numeric prefix search, so a partial id cannot bypass the minimum-length rule. Any other query becomes a bounded text search by username/nickname/effective name, which requires only the already-enabled `GUILD_MEMBERS` intent (no `MESSAGE_CONTENT` or extra privileged permissions). The `JdaGuildModerationDiscordClient` converts JDA members into the moderation-owned `MemberSearchResultMember` DTO so no JDA type crosses into `guildmoderation`, maps unresolvable guilds and Discord failures to the same controlled `ModerationDiscordActionException` categories, and treats not-found as an empty result. Searches are read-only and never create audit events. Guild OS does not persist member names, display names, avatar URLs, or search queries: the returned `username`/`displayName` are transient selection metadata, and adapter logs carry only the guild id and failure category — never the query text, usernames, avatars, or raw Discord payloads.
