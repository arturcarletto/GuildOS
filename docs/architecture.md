...
The `guildmoderation` capability owns the first moderation action foundation: creating a Discord member communication timeout from the authenticated dashboard API, plus a live member search that helps operators select a target. It is intentionally narrow and does not include warnings, kick, ban, message deletion, queues, AI moderation, appeal workflows, realtime moderation streams, a persistent member directory or member profile table, avatar storage, role management, or a dedicated moderation action history table beyond the guild audit log.
